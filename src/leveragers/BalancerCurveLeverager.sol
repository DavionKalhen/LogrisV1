    ...
    */
    ...
    function leverage(uint clampedDeposit, uint flashLoanAmount, uint underlyingDepositMin, uint mintAmount, uint debtTradeMin) public {
        ...
        if(flashLoanAmount == 0) {
            ...
            bytes memory data = abi.encode(msg.sender, true, clampedDeposit, flashLoanAmount, underlyingDepositMin, mintAmount, debtTradeMin);
            vault.flashLoan(this, tokens, amounts, data);
        }
        //the dust should get transmitted back to msg.sender but it might not be worth the gas...
    }

    function _getFlashLoanParameters(uint flashLoanAmount) internal view returns (IERC20[] memory tokens, uint[] memory amounts) {
        tokens = new IERC20[](1);
    ...
    function receiveFlashLoan(IERC20[] memory, uint256[] memory, uint256[] memory, bytes memory userData) external override {
        ...
        (address depositor, bool depositFlag, uint param1, uint param2, uint param3, uint param4, uint param5) = abi.decode(userData, (address, bool, uint, uint, uint, uint, uint));
        if(depositFlag) {
    ...
    function _flashLoanDeposit(address depositor, uint clampedDeposit, uint flashLoanAmount, uint underlyingDepositMin, uint mintAmount, uint debtTradeMin) internal {
    ...
    function withdrawUnderlying(uint shares, uint flashLoanAmount, uint burnAmount, uint debtTradeMin, uint minUnderlyingOut) public {
        ...
        if(burnAmount==0) {
            ...
            (IERC20[] memory tokens, uint[] memory amounts) = _getFlashLoanParameters(flashLoanAmount);
            bytes memory data = abi.encode(msg.sender, false, shares, flashLoanAmount, burnAmount, debtTradeMin, minUnderlyingOut);
            vault.flashLoan(this, tokens, amounts, data);
        }
    }

    function _flashLoanWithdraw(address depositor, uint shares, uint flashLoanAmount, uint burnAmount, uint debtTradeMin, uint minUnderlyingOut) internal {
    ...
    function _withdrawUnderlying(address depositor, uint shares, uint minUnderlyingOut) internal {
        uint underlying = alchemist.withdrawUnderlyingFrom(depositor, yieldToken, shares, address(this), minUnderlyingOut);
