...
contract Leverager is Ownable {
    ...
    function totalValue(address [] memory values) internal view returns (uint256 totalValue_) {
        ...
        for (uint256 i = 0; i < values.length; i++) {
            ...
            (uint256 shares, )             = alchemist.positions(address(this), yieldToken);
            uint256 amountUnderlyingTokens = _convertSharesToUnderlyingTokens(yieldTokenParams, shares);

            totalValue_ += _normalizeUnderlyingTokensToDebt(underlyingToken, amountUnderlyingTokens);
        }
    }
    ...
    function _normalizeUnderlyingTokensToDebt(address underlyingToken, uint256 amount) internal view returns (uint256) {
        IAlchemistV2.UnderlyingTokenParams memory underlyingTokenParams = alchemist.getUnderlyingTokenParameters(underlyingToken);
